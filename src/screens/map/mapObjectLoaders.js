...
export function loadEncounterZones(map, encounterLayer) {
  ...
  // Plafond de numéro d'instinct pour cette map (propriété custom du .tmj). Si défini,
  // le second tirage (l'instinct attaché au fawna) n'offrira pas d'instinct dont le
  // numéro dépasse cette valeur.
  const mapMaxInstinctNumber = propertiesFromObject(map).maxInstinctNumber;
  ...
  return [
    ...
    ...zone,
    maxCreatureNumber: mapMaxCreatureNumber,
    maxInstinctNumber: mapMaxInstinctNumber
  }));
}
